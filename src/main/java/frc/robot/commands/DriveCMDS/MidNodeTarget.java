...
public class MidNodeTarget extends SequentialCommandGroup {
    ...
    private double coneAngleBackset = -10;
    private double cubeAngleBackset = coneAngleBackset;
     ...
     */
    public MidNodeTarget(DriveTrain m_drive, CommandXboxController driveController, CommandXboxController opController) {
        ...
        addCommands(
           ...
           /*Og*/ //Commands.run(() -> m_drive.drive(translate.calculate(MathUtil.applyDeadband(driveController.getLeftY(), 0.3)), kStrafingPID.calculate(Limelight.x, 0), kturningPID.calculate(m_drive.getHeadingDegrees(), 0), false)
            /*Distance Applied (0 for high)*/ Commands.run(() -> m_drive.drive(kDrivingPID.calculate(Limelight.y, coneAngleBackset), kStrafingPID.calculate(Limelight.x, 0), kturningPID.calculate(m_drive.getHeadingDegrees(), 0), false)
